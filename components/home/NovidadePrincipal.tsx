...
const NovidadePrincipal = async () => {
  ...
  return (
    ...
    <Transition className='w-full h-full'>
        <Link  href={`/home/news/${novidade.id}`} className='duration-500 transition-all ease-in-out hover:scale-[100.5%] relative sm:gap-0 gap-20 overflow-hidden w-full rounded-lg h-full flex flex-col items-start justify-between p-5'>

            <h3 className='uppercase font-semibold bg-slate-700 text-[13px] rounded-lg px-2 py-1 min-w-[100px] text-center'>Novidade</h3>

            <div className=' w-full h-full flex flex-col items-start justify-end gap-3'>
              <h1 className='font-bold text-xl'>{novidade.titulo}</h1>
              <BotaoWithWhere  text="Visualizar" where={`home/news/${novidade.id}`}/>
            </div>

            <Image fill className='object-top object-cover top-0 left-0 bottom-0 right-0 z-[-1] absolute brightness-50  w-full h-full'  alt={`Capa da notícia ${novidade.id}`} src={novidade.capa}/>
